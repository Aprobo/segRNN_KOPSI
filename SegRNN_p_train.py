...
# 데이터 로드 및 일별 데이터로 변환
data = pd.read_csv('./data/kospi_p.csv')
data['Date'] = pd.to_datetime(data['Date']) #Dtype: object -> dateimte64
print(data.tail)
data.set_index('Date', inplace=True)
data.rename(columns={"KOSPI": "Price"}, inplace=True)
...
# 세그먼트 기반 데이터셋 생성
train_segments = segment_data(train_scaled, segment_length) #[X/30,30,1]
...
def create_segmented_dataset(segments, base_start_idx, segment_length, look_back):
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        for i in range(len(segment) - look_back):
            X.append(segment[i:(i + look_back)])   # (look_back, 1)
            Y.append(segment[i + look_back])       # (1,)
...
if __name__ == "__main__":
    ...
    test_act   = scaler.inverse_transform(test_act)
   ...
   # 마지막 몇 일의 결과 출력
    ...
    # 실제 KOSPI
    plt.plot(dates, values.reshape(-1), label='Actual KOSPI', color='black')
    ...
    plt.ylabel('KOSPI')
    plt.legend()
    plt.savefig('kospi_daily_prediction.png')  # 그래프를 이미지로 저장
    plt.show()
    ...
    plt.savefig('kospi_test_prediction.png')
    plt.show()
    ...
    plt.savefig('loss_over_epochs_log_scale.png')  # 그래프를 이미지로 저장
    plt.show()